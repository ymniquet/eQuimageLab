...
.. note::
  This convention differs from the usual representation of images as arrays with shape (H, W, 3) or (H, W). It is, however, more convenient to represent the image as such a superposition of "layers" or "channels" for many operations.
...
An exisiting image represented as a :py:class:`numpy.ndarray` array `imgarray` can be embedded in an :py:class:`Image <equimagelab.equimage.image.Image>` object using the constructor ``image = Image(imgarray)``. You can specify the channel axis for color images [``image = Image(imgarray, channels = 0)`` if `imgarray` has shape (3, H, W), and ``image = Image(imgarray, channels = -1)`` if `imgarray` has shape (H, W, 3)].
